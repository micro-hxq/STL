#ifndef _STL_WT_UNORDERED_MULTISET_HPP_
#define _STL_WT_UNORDERED_MULTISET_HPP_
...
public:
    ...
    unordered_multiset() : unordered_multiset(wt_default_bucket_count()) { }
    explicit unordered_multiset(size_type _bcount,
                                const hasher& _hash = hasher(),
                                const key_equal& _equal = key_equal(),
                                const allocator_type& _alloc = allocator_type())
    : m_impl_(_bcount, _hash, _equal, _alloc) { }

    unordered_multiset(size_type _bcount, const allocator_type& _alloc)
    : m_impl_(_bcount, hasher(), key_equal(), _alloc) { }

    unordered_multiset(size_type _bcount, const hasher& _hash, 
                       const allocator_type& _alloc)
    : m_impl_(_bcount, _hash, key_equal(), _alloc) { }

    explicit unordered_multiset(const allocator_type& _alloc)
    : m_impl_(wt_default_bucket_count(), hasher(), key_equal(), _alloc) { }

    template <typename InputIter>
    unordered_multiset(InputIter _first, InputIter _last,
    ...
    unordered_multiset(const unordered_multiset& other, const allocator_type& _alloc)
    ...
    unordered_multiset& operator=(std::initializer_list<value_type> _ilist)
    {
        m_impl_.clear();
        m_impl_.insert_equal(_ilist.begin(), _ilist.end());
        return *this;
    ...
    friend bool operator==(const unordered_multiset<_K, _H, _KE, _A>&,
                           const unordered_multiset<_K, _H, _KE, _A>&);
private:
    _Impl_type  m_impl_;
};
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
bool
operator==(const unordered_multiset<Key, Hash, KeyEqual, Allocator>& lhs,
...
template <typename Key, typename Hash, typename KeyEqual, typename Allocator>
void
swap(const unordered_multiset<Key, Hash, KeyEqual, Allocator>& lhs,
     const unordered_multiset<Key, Hash, KeyEqual, Allocator>& rhs)
{
    lhs.swap(rhs);
}
