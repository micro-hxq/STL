#ifndef _STL_WT_MAP_HPP_
#define _STL_WT_MAP_HPP_
...
public:
    ...
    map() : map(key_compare()) { }
    explicit map(const key_compare& _comp, 
                 const allocator_type& _alloc = allocator_type())
    : m_impl_(_comp, _alloc) { }

    explicit map(const allocator_type& _alloc)
    : m_impl_(key_compare(), _alloc) { }

    template <typename InputIter>
    map(InputIter _first, InputIter _last, 
    ...
    template <typename InputIter>
    map(InputIter _first, InputIter _last, const allocator_type& _alloc)
    : m_impl_(key_compare(), _alloc)
    {
        m_impl_.insert_unique(_first, _last);
    }
    ...
    map(const map& other, const allocator_type& _alloc)
    ...
    map(std::initializer_list<value_type> _ilist, const allocator_type& _alloc)
    : m_impl_(key_compare(), _alloc)
    {
        m_impl_.insert_unique(_ilist.begin(), _ilist.end());
    }
    ...
    mapped_type& operator[](const key_type& _key)
    {
        ...
        if(pos == end() || key_compare()(_key, pos->first))
            pos = m_impl_.insert_unique(pos, value_type(_key, mapped_type()));
        return pos->second;
    }
    ...
    friend bool 
    operator==(const map<_K, _T, _C, _A>&, const map<_K, _T, _C, _A>&);
    template <typename _K, typename _T, typename _C, typename _A>
    friend bool
    operator<(const map<_K, _T, _C, _A>&, const map<_K, _T, _C, _A>&);
...
template <typename Key, typename T, typename Compare, typename Allocator>
void
swap(map<Key, T, Compare, Allocator>& lhs, 
